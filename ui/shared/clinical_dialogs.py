...
class NewInvoiceDialog(QDialog):
    ...
    def __init__(self, parent=None, *, services=None, payment_methods=None,
        ...
        # Items table (Service, Unit Price, Discount, Subtotal, Remove)
        ...
        totals_frame = QFrame()
        totals_frame.setStyleSheet("QFrame { border-top: 2px solid #E8F0F1; }")
        totals_frame.setMinimumWidth(220)
        
        totals_col = QVBoxLayout(totals_frame)
        totals_col.setContentsMargins(0, 10, 0, 0)
        totals_col.setSpacing(4)
        
        self._subtotal_lbl = QLabel("Subtotal: ₱0.00")
        self._subtotal_lbl.setStyleSheet("font-size: 13px; color: #7F8C8D; border: none;")
        self._subtotal_lbl.setAlignment(Qt.AlignmentFlag.AlignRight)
        self._discount_lbl = QLabel("Discount: − ₱0.00")
        self._discount_lbl.setStyleSheet("font-size: 13px; color: #D9534F; border: none;")
        self._discount_lbl.setAlignment(Qt.AlignmentFlag.AlignRight)
        self._total_lbl = QLabel("Total: ₱0.00")
        self._total_lbl.setStyleSheet("font-weight: bold; font-size: 16px; color: #388087; border: none;")
        ...
        totals_row.addWidget(totals_frame)
    ...
    def _on_patient_changed(self, text: str):
        ...
        # Update discount badge and lock state
        if self._patient_discount_pct > 0 and self._patient_discount_type:
            ...
            # Unconditionally lock the discount spinbox if automatic discount exists
            self.disc_spin.setReadOnly(True)
            self.disc_spin.setEnabled(False)
            ...
            # Unlock the discount spinbox only if Admin and no automatic discount exists
            if self._role == "Admin":
                self.disc_spin.setReadOnly(False)
                self.disc_spin.setEnabled(True)
            self.disc_spin.setValue(0.0)
    ...
    def _add_line(self):
        ...
        # Use locked discount if patient has one, otherwise spinbox
        if self._patient_discount_pct > 0 or self._role != "Admin":
        ...
        self._items_table.setItem(r, 1, QTableWidgetItem(f"₱{price:,.2f}"))
        disc_item = QTableWidgetItem(f"{disc:.0f}%")
        if disc > 0:
            disc_item.setForeground(QColor("#D9534F"))
        self._items_table.setItem(r, 2, disc_item)
        self._items_table.setItem(r, 3, QTableWidgetItem(f"₱{sub:,.2f}"))
        # Remove button
        rm_btn = QPushButton("✕")
        ...
        self._items_table.setCellWidget(r, 4, rm_btn)
        self._update_totals()
        if self._role == "Admin" and self._patient_discount_pct == 0:
            self.disc_spin.setValue(0.0)
    ...
    def _recalculate_all_items(self):
        ...
        for i, it in enumerate(self._line_items):
            ...
            if disc > 0:
                disc_item.setForeground(QColor("#D9534F"))
            self._items_table.setItem(i, 2, disc_item)
            self._items_table.setItem(i, 3, QTableWidgetItem(f"₱{it['subtotal']:,.2f}"))
        self._update_totals()
    ...
    def _validate_and_accept(self):
        ...
        # UX logic: Auto-add the selected line item if the user hit Create without clicking +Add
        if not self._line_items and self.svc_combo.currentData():
            self._add_line()
