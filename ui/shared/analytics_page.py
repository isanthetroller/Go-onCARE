...
class AnalyticsPage(QWidget):
    ...
    def _pie_card(title_text, subtitle_text, data) -> QFrame:
        ...
        chart_col.setSpacing(0)
        chart = PieChartWidget(data); chart.setFixedSize(200, 200)
        chart_col.addWidget(chart, alignment=Qt.AlignmentFlag.AlignCenter)
        
        total = sum(v for _, v, _ in data)
        c_total = QLabel(f"Total: {total:,}")
        c_total.setStyleSheet("font-weight: bold; font-size: 16px; color: #1A252F;")
        c_total.setAlignment(Qt.AlignmentFlag.AlignCenter)
        chart_col.addWidget(c_total)
        
        content.addLayout(chart_col)
    ...
    def _summary_table_card(self, data: dict) -> QFrame:
        ...
        self.sum_from = QDateEdit(); self.sum_from.setCalendarPopup(True); apply_modern_calendar(self.sum_from)
        ...
        self.sum_to = QDateEdit(); self.sum_to.setCalendarPopup(True); apply_modern_calendar(self.sum_to)
