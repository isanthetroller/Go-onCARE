...
def make_banner(title_text: str, subtitle_text: str,
    ...
    """Build a premium page banner with gradient and depth.
    ...
    banner.setStyleSheet("""
        QFrame#pageBanner {
            background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
                stop:0 #2C6A70, stop:0.5 #388087, stop:1 #3D9098);
            border-radius: 16px;
            border: none;
        }
    """)
    ...
    shadow.setColor(QColor(44, 106, 112, 50))
    banner.setGraphicsEffect(shadow)

    banner_lay = QHBoxLayout(banner)
    banner_lay.setContentsMargins(36, 32, 36, 28)
    ...
    t.setStyleSheet(
        ...
        " background: transparent; padding: 0; letter-spacing: -0.3px;"
    )
    tc.addWidget(t)

    # Accent separator — wider, more visible
    sep = QFrame()
    sep.setFixedHeight(2)
    sep.setFixedWidth(60)
    sep.setStyleSheet("background: rgba(255,255,255,0.35); border: none; border-radius: 1px;")
    tc.addWidget(sep)
    ...
    s.setStyleSheet(
        ...
        " background: transparent; padding: 2px 0 0 0; font-weight: 500;"
    )
    tc.addWidget(s)
