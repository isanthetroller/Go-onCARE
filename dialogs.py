...
# â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?
#  Service Edit Dialog â?" V3 (gradient header, polished form)
# â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?â?
class ServiceEditDialog(QDialog):
    ...
    def __init__(self, parent=None, data=None, categories=None, departments=None, selected_departments=None):
        ...
        self.dept_list_widget.setSelectionMode(QListWidget.SelectionMode.NoSelection)
        self.dept_list_widget.setFixedHeight(120)
        self.dept_list_widget.setStyleSheet(
            "QListWidget { border: 2px solid #BADFE7; border-radius: 8px; padding: 4px; font-size: 13px; }"
        )
        self.departments = departments or []
        selected_deps = set(selected_departments or [])
        for dept in self.departments:
            item = QListWidgetItem(dept["department_name"])
            item.setData(Qt.ItemDataRole.UserRole, dept["department_id"])
        ...
        if self.departments:
            form.addRow("Departments",  self.dept_list_widget)
    ...
    def get_data(self) -> dict:
        ...
        for i in range(self.dept_list_widget.count()):
            ...
            if item.checkState() == Qt.CheckState.Checked:
                selected_deps.append(item.data(Qt.ItemDataRole.UserRole))
...
class NewInvoiceDialog(QDialog):
    ...
    def __init__(self, parent=None, *, services=None, payment_methods=None,
        ...
        self._discount_lbl = QLabel("Discount: â^' â,±0.00")
    ...
    def _on_patient_changed(self, text: str):
        ...
        if self._patient_discount_pct > 0 and self._patient_discount_type:
            self._discount_badge.setText(
                f"dY?·  {self._patient_discount_type} â?" {self._patient_discount_pct:.0f}% discount applied automatically")
            self._discount_badge.setStyleSheet(
    ...
    def _update_totals(self):
        ...
        self._discount_lbl.setText(f"Discount: â^' â,±{disc_total:,.2f}")
